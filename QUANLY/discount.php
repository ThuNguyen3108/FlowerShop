...
if (isset($_POST['submited'])) {
    $resultChiTiet2 = $conn->query($sql);
    while ($chitiet = $resultChiTiet2->fetch_assoc()) {
        $giamgia = $_POST[$chitiet['MSHH']];
        $sqlUpdate = "update chitietdathang set GiamGia=$giamgia where MSHH = '$chitiet[MSHH]' and SoDonDH = '$SoDonDH'";
            ...
            <form method="POST">
                ...
                <table class="table" style="font-size:20px;width:100%">
                    ...
                    <tbody>
                        ...
                        <?php
                        while ($chitiet = $resultChiTiet->fetch_assoc()) {
                            $resultHH = $conn->query("Select * from hanghoa where MSHH = '$chitiet[MSHH]'");
                            $hanghoa = $resultHH->fetch_assoc();
                            $value = 0;
                            if (isset($chitiet['GiamGia'])) {
                                $value = $chitiet['GiamGia'];
                            }

                        ?>
                            <tr>
                                <td scope="row"><?= $hanghoa['MSHH'] ?></td>
                                <td><?= $hanghoa['TenHH'] ?></td>
                                <td><?= $chitiet['GiaDatHang'] ?></td>
                                <td>
                                    <input value='<?= $value ?>' name="<?= $chitiet['MSHH'] ?>" type="number" class="form-control" placeholder="Giá sẽ giảm">
                                </td>
                            </tr>
                        <?php
                        }
                ...
                <input type="text" name="submited" value="yes" style="display:none" />
